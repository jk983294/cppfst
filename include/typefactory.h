...
public:
    Blob(unsigned long long size) { pData = new unsigned char[size]; }

    unsigned char* Data() { return pData; }
...
public:
    ...
    unsigned char* Data() { return shared_data->Data(); }

    std::shared_ptr<Blob> DataPtr() const { return shared_data; }

    unsigned long long Size() { return blobSize; }
};
...
public:
    ...
    IBlobContainer* CreateBlobContainer(unsigned long long size) { return new BlobContainer(size); }
};
