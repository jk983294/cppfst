...
enum FstColumnAttribute {
    ...
    DOUBLE_64_TIMEINTERVAL_SECONDS,  // number of fractional seconds between two moments in time [scale is FstTimeScale
                                     // enum]
...
// Default scale
// Only add to the end to support previous format versions
enum FstScale { UNIT = 0 };

// Available time scales
// Only add to the end to support previous format versions
enum FstTimeScale { NANOSECONDS = 1, MICROSECONDS, MILLISECONDS, SECONDS, MINUTES, HOURS, DAYS, YEARS };
...
public:
    ...
    virtual void BufferToVec(uint64_t nrOfElements, uint64_t startElem, uint64_t endElem, uint64_t vecOffset,
                             unsigned int* sizeMeta, char* buf) = 0;
