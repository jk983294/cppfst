...
public:
    StringVector(uint64_t vecLength) { this->data = new std::vector<std::string>(vecLength); }

    StringVector(std::vector<std::string>& vec) {
        data = &vec;
        m_borrow = true;
    }

    ~StringVector() override { if (not m_borrow) delete data; }
...
class StringColumn : public IStringColumn {
    ...
    StringEncoding string_encoding = StringEncoding::UTF8;

public:
    StringColumn() {}

    StringColumn(std::vector<std::string>& vec) {
        shared_data = std::make_shared<StringVector>(vec);
    }
    ...
    void SetEncoding(StringEncoding stringEncoding) { this->string_encoding = stringEncoding; }

    StringEncoding GetEncoding() { return string_encoding; }
...
public:
    ByteVector(uint64_t length) { this->data = new char[length]; }

    ~ByteVector() { delete[] data; }
...
public:
    LongVector(uint64_t length) { this->data = new long long[length]; }

    ~LongVector() { delete[] data; }

    long long* Data() { return data; }
};
...
public:
    DoubleVector(uint64_t length) { this->data = new double[length]; }
    ...
    ~DoubleVector() override { if (not m_borrow) delete[] data; }
...
public:
    ...
    StringColumn* Levels() const { return levels; }
};
...
public:
    ...
    byte_block_array_ptr* blocks() { return byte_blocks.get(); }

    uint64_array_ptr* sizes() { return block_sizes.get(); }
    ...
    void SetSizesAndPointers(const char** elements, uint64_t* sizes, uint64_t row_start, uint64_t block_size) {
        ...
        std::memcpy(elements, start_block_address, block_size * 8);
        std::memcpy(sizes, start_size_address, block_size * 8);
    }
};
...
public:
    ...
    FstColumnAttribute Attribute() { return columnAttribute; }

    short int Scale() { return scale; }

    int* Data() { return shared_data->Data(); }

    std::shared_ptr<DestructableObject> DataPtr() const { return shared_data; }

    void Annotate(std::string annotation) { this->annotation = annotation; }
};
...
public:
    ...
    char* Data() { return shared_data->Data(); }
...
public:
    ...
    long long* Data() { return shared_data->Data(); };

    std::shared_ptr<LongVector> DataPtr() const { return shared_data; }

    FstColumnAttribute ColumnAttribute() const { return columnAttribute; }
};
...
public:
    LogicalVectorAdapter(uint64_t length) { shared_data = std::make_shared<IntVector>(std::max(length, (uint64_t)1)); }

    ~LogicalVectorAdapter() {}

    int* Data() { return shared_data->Data(); }

    std::shared_ptr<IntVector> DataPtr() const { return shared_data; }
};
...
public:
    ...
    int* LevelData() { return shared_data->Data(); }

    IStringColumn* Levels() { return shared_data->Levels(); }

    std::shared_ptr<FactorVector> DataPtr() const { return shared_data; }
};

class DoubleVectorAdapter : public IDoubleColumn {
    std::shared_ptr<DoubleVector> shared_data;
    FstColumnAttribute columnAttribute = FstColumnAttribute::DOUBLE_64_BASE;
    std::string annotation;
    short int scale{0};
...
public:
    ...
    double* Data() { return shared_data->Data(); }

    std::shared_ptr<DoubleVector> DataPtr() { return shared_data; }

    void Annotate(std::string annotation) { this->annotation = annotation; }

    std::string Attribute() const { return annotation; }
};
...
public:
    ...
    StringArray(std::vector<std::string>& strVector) { SetArray(strVector); }
    ...
    std::string GetStringElement(uint64_t index) { return strVec->at(index); }
    ...
    const char* GetElement(uint64_t elementNr) { return (*strVec)[elementNr].c_str(); }

    uint64_t Length() { return (uint64_t)strVec->size(); }

    void SetEncoding(StringEncoding string_encoding) { this->string_encoding = string_encoding; }
};
...
public:
    ...
    StringEncoding Encoding() { return StringEncoding::LATIN1; }
};
...
public:
    FstTable() { this->nrOfRows = 0; }

    FstTable(uint64_t nrOfRows) { this->nrOfRows = nrOfRows; }
    ...
    FstTable* SubSet(std::vector<std::string>& columnNames, uint64_t startRow, uint64_t endRow) const {
        ...
        for (std::vector<std::string>::iterator colIt = columnNames.begin(); colIt != columnNames.end(); ++colIt) {
            ...
            subset->SetColumn((*columns)[index], subSetColNr, (*columnTypes)[index], (*columnAttributes)[index],
                              (*colNames)[index], (*colScales)[index], (*colAnnotations)[index]);
            subSetColNr++;
        }
    ...
    void SetColumn(std::shared_ptr<DestructableObject> column, int colNr, FstColumnType type,
                   FstColumnAttribute attribute, std::string colName, short int scale, std::string annotation) const {
    ...
    std::vector<std::string>* ColumnNames() { return this->colNames; }

    void SetColumnNames(StringArray& colnames) const {
        for (size_t colNr = 0; colNr < this->colNames->size(); colNr++) {
    ...
    void SetColumnNames(std::vector<std::string> colNameVec) const {
        int colNr = 0;
        for (auto colNameIt = colNameVec.begin(); colNameIt != colNameVec.end(); ++colNameIt) {
    ...
    IByteBlockColumn* GetByteBlockWriter(uint32_t col_nr) {
        ...
        std::shared_ptr<ByteBlockVectorAdapter> byte_block =
            std::static_pointer_cast<ByteBlockVectorAdapter>(byte_block_writer);
    ...
    uint32_t NrOfKeys() { return 0; }

    uint32_t NrOfColumns() { return (uint32_t)columnTypes->size(); }
