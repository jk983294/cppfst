...
enum COMPRESSION_ALGORITHM { ALGORITHM_NONE = 0, ALGORITHM_LZ4, ALGORITHM_ZSTD };
...
public:
     ...
     */
    FstCompressor(const COMPRESSION_ALGORITHM comp_algorithm, const unsigned int compressionLevel,
                  ITypeFactory* typeFactory) {
        this->typeFactory = typeFactory;
     ...
     */
    IBlobContainer* CompressBlob(unsigned char* blob_source, const unsigned long long blob_length,
                                 const bool hash = true) const {
        ...
        const unsigned long long min_block =
            std::max(static_cast<unsigned long long>(BLOCKSIZE), 1ULL + (blob_length - 1ULL) / PREV_NR_OF_BLOCKS);
        ...
        const unsigned long long block_size =
            static_cast<unsigned long long>(std::min(min_block, static_cast<unsigned long long>(0.8 * INT_MAX)));
...
#pragma omp for schedule(static, 1) nowait
            for (int block_batch = 0; block_batch < (nr_of_threads - 1); block_batch++)  // all but last batch
            {
                ...
                for (int block = block_nr; block < next_block_nr; block++) {
                    // error when > 4 GB !
                    const int comp_size = this->compressor->Compress(
                        reinterpret_cast<char*>(thread_buf + buf_pos), max_compress_size,
                        reinterpret_cast<char*>(&blob_source[block_size * static_cast<unsigned long long>(block)]),
                        block_size, comp_algo);
...
#pragma omp single
            {
                ...
                const int block_nr = static_cast<int>(DOUBLE_DELTA + block_offset);
                const int next_block_nr =
                    static_cast<int>(blocks_per_thread + DOUBLE_DELTA + block_offset) - 1;  // exclude last block
                ...
                const int comp_size = this->compressor->Compress(
                    reinterpret_cast<char*>(&thread_buf[buf_pos]), max_compress_size,
                    reinterpret_cast<char*>(&blob_source[block_size * static_cast<unsigned long long>(next_block_nr)]),
                    last_block_size, comp_algo);
...
#pragma omp parallel for schedule(static, 1)
        for (int blockBatch = 0; blockBatch < nr_of_threads; blockBatch++) {
            ...
            unsigned char* threadBuf = calc_buffer + static_cast<unsigned long long>(max_compress_size) *
                                                         blockNr;  // buffer for compression results of current thread
...
#pragma omp single
                {
                    const int fromBlock =
                        static_cast<int>((nrOfThreads - 1) * batchFactor + DOUBLE_DELTA);            // start block
...
#pragma omp for schedule(static, 1) nowait
            for (int batch = 0; batch < (nrOfThreads - 1); batch++)  // all but last batch
            {
                ...
                for (int block = fromBlock; block < toBlock; block++) {
                    ...
                    const unsigned int errorCode = decompressor.Decompress(
                        algorithm, reinterpret_cast<char*>(blob_data) + *blockSize * block, *blockSize,
...
#pragma omp single
            {
                ...
                const unsigned long long blockEnd = blockOffsets[toBlock];
                const unsigned int errorCode = decompressor.Decompress(
                    algorithm, reinterpret_cast<char*>(blob_data) + *blockSize * (toBlock - 1), lastBlockSize,
                    reinterpret_cast<const char*>(blobSource + blockStart), blockEnd - blockStart);
