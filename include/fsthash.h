...
public:
    ...
    uint64_t HashBlobSeed(unsigned char* blobSource, uint64_t blobLength, uint64_t seed, bool blockHash = true) const {
        ...
        uint64_t minBlock = std::max(static_cast<uint64_t>(HASH_SIZE), 1 + (blobLength - 1) / PREV_NR_OF_BLOCKS);
...
#pragma omp single
            {
                ...
                int nextblockNr =
                    static_cast<int>(0.00001 + (nrOfThreads * blocksPerThread)) - 1;  // exclude last block
